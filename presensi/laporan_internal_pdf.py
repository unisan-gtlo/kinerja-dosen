...
def _label_filter(kategori, fakultas, prodi):
    """`fakultas`/`prodi` yang masuk ke sini adalah KODE (nilai dropdown
    form, mis. "FK"), bukan nama lengkap -- di-resolve ke nama lengkap
    lewat master.Fakultas/Prodi di sini, dipakai bareng oleh Laporan
    Presensi Internal & Laporan Bulanan Jam Kerja (PDF+Excel keduanya
    import fungsi ini)."""
    ...
    if fakultas:
        f = Fakultas.objects.filter(kode_fakultas=fakultas).first()
        bagian.append(f"Fakultas: {f.nama_fakultas if f else fakultas}")
    if prodi:
        p = Prodi.objects.filter(kode_prodi=prodi).first()
        bagian.append(f"Prodi: {p.nama_prodi if p else prodi}")
    return " · ".join(bagian)
...
def render_pdf_laporan_internal(
    ...
    cell_tengah_style = ParagraphStyle("cell_tengah", parent=cell_style, alignment=TA_CENTER)
    jam_style = ParagraphStyle("jam", parent=styles["Normal"], fontSize=5, alignment=TA_CENTER, leading=5.5)
    ...
    lebar_no, lebar_nama, lebar_prodi = 0.7 * cm, 3.6 * cm, 2.6 * cm
    ...
    label_kolom_instansi = {"dosen": "Program\nStudi", "tendik": "Unit\nKerja"}.get(kategori, "Prodi /\nUnit Kerja")
    header_row = ["No", "Nama", label_kolom_instansi] + [str(tanggal.day) for tanggal, _jenis in jenis_list]
    table_data = [header_row]

    for idx, baris in enumerate(daftar, start=1):
        nama = Paragraph(baris["user"].nama_resmi, cell_style)
        instansi = Paragraph(baris["nama_prodi"] or "-", cell_tengah_style)
        sel_tanggal = [
        ...
        table_data.append([str(idx), nama, instansi] + sel_tanggal)
    ...
    table.setStyle(TableStyle([
        ...
        # LEFT/RIGHTPADDING default reportlab 6pt per sisi -- di kolom
        # tanggal yang cuma ~0.78cm (±22pt) lebar totalnya, itu memakan
        # HAMPIR SEMUA ruang (12pt dari 22pt), menyisakan ruang teks
        # terlalu sempit utk "10:48" bahkan di fontSize kecil -- makanya
        # sebelumnya tetap pecah jadi 4 baris walau font sudah diperkecil.
