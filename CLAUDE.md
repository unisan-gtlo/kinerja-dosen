...
- ✅ **Kategori** — `presensi/laporan_internal.py::KATEGORI_ROLES` = `{"dosen": ["dosen"], "pejabat": ["dekan","wadek","kaprodi","sekprodi","rektorat","biro"], "tendik": ["tendik"]}` (persis definisi role "Pejabat" yang sudah dipakai `KelompokPresensi`). Role `admin`/`operator` sengaja tidak masuk kategori manapun (konsisten dengan seed `KelompokPresensi`) — tetap muncul kalau filter kategori dikosongkan.
