...
- ⚠️ **Staf/tendik BELUM tercakup di presensi** — seluruh skema (`Presensi`, `JadwalKerja`, `Perangkat`, `EnrolmentWajah`, `IzinCuti`, `LogKecurangan`) dan dashboard/rekap di atas masih **dosen-only**, dikunci `nidn` (yang cuma dimiliki dosen). User sudah konfirmasi staf **wajib** tercakup nanti — butuh migrasi kunci dari `nidn` (CharField) ke `ForeignKey(accounts.User)` di 6 model tsb (NIDN jadi pengayaan data, bukan kunci utama). **Belum dikerjakan**, disepakati dikerjakan menyusul, jangan asumsikan dashboard/rekap saat ini sudah mencakup staf.
